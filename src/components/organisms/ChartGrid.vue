...
import { getRecordsFetch, getCadenceFetch } from '@/core/record/recordApi'
...
const defaultCharts = [
  {
    id: 1,
    title: '운동 거리 추이',
    description: '선택한 기간 동안의 일별 운동 거리를 보여줍니다. 꺾은선 그래프를 통해 운동량의 변화 추이를 쉽게 파악할 수 있습니다.'
  },
  {
    id: 2,
    title: '시간대별 운동 분포',
    description: '하루 중 어느 시간대에 운동을 많이 하는지 보여줍니다. 24시간을 기준으로 운동 빈도를 시각화하여 선호하는 운동 시간대를 파악할 수 있습니다.'
  },
  {
    id: 3,
    title: '페이스 구간별 분석',
    description: '운동 중 각 페이스 구간에서 보낸 시간을 보여줍니다. 산책부터 빠른 러닝까지 각 구간별 운동 시간 분포를 확인할 수 있습니다.'
  },
  {
    id: 4,
    title: '운동 통계',
    description: '선택한 기간의 주요 운동 지표를 한눈에 보여줍니다. 총 운동 횟수, 이동 거리, 소모한 칼로리, 평균 심박수 등의 핵심 정보를 확인할 수 있습니다.'
  },
  {
    id: 5,
    title: '평균 케이던스 추이',
    description: '일별 평균 케이던스(분당 발걸음 수) 추이를 보여줍니다. 케이던스는 러닝 효율성을 나타내는 중요한 지표로, 일반적으로 160-180 steps/min이 효율적인 러닝 케이던스로 알려져 있습니다.'
  },
  {
    id: 6,
    title: '현재 날씨',
  ...
  },
  {
    id: 7,
    title: '심박수와 속도 관계',
    description: '운동 중 심박수와 속도의 관계를 산점도로 보여줍니다. 이를 통해 운동 강도와 심박수의 상관관계를 분석할 수 있습니다.'
  },
  {
    id: 8,
    title: '심박수 구간별 운동량',
    description: '운동 중 각 심박수 구간에서 보낸 시간을 보여줍니다. 휴식, 준비운동, 지방연소, 유산소, 무산소 등 각 심박존별 운동 시간 분포를 확인할 수 있습니다.'
  },
...
watch(() => props.dateRange, async ([start, end]) => {
  ...
  try {
    ...
    if (Array.isArray(cadenceResponse.data)) {
      console.log(cadenceResponse.data);
    ...
    console.log('Fetched cadence data:', cadenceData.value)
  } catch (error) {
...
<template>
  <div class="grid grid-cols-3 gap-4 auto-rows-[minmax(25vh,auto)]">
    <draggable
      v-model="charts"
      class="contents"
      item-key="id"
    ...
    >
      <template #item="{ element }">
        <div
          :class="[
            'bg-white p-4 rounded-2xl shadow-sm cursor-move hover:shadow-md transition-shadow relative z-0',
            getClass(element.id)
          ]"
        >
          <div class="h-full">
            <!-- 차트 정보 아이콘 -->
            <div v-if="element.description" class="absolute top-3 right-3">
